...
void option_mode::Finish() {
    ...
    if (option_background) {
        ...
        option_background = nullptr;
    }

    for (int i = 0; i < 10; i++) {
        if (bgm_volume_image[i]) {
            delete bgm_volume_image[i];
            bgm_volume_image[i] = nullptr;
        }
        if (effect_volume_image[i]) {
            delete effect_volume_image[i];
            effect_volume_image[i] = nullptr;
        }
    }

    if (shaderProgramImage) {
        glDeleteProgram(shaderProgramImage);
        shaderProgramImage = 0;
    }
